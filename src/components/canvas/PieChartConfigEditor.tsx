...
import type { ChartAggregation, PieChartConfig } from '../../types';

interface PieChartConfigEditorProps {
  initialConfig: PieChartConfig;
  onSubmit: (config: PieChartConfig) => void;
  onClose: () => void;
  submitLabel: string;
}
...
export function PieChartConfigEditor({
  initialConfig,
  onSubmit,
  onClose,
  submitLabel,
}: PieChartConfigEditorProps) {
  ...
  return (
    <div className="w-80 bg-white text-gray-900 border border-gray-200 rounded-lg shadow-xl p-3 space-y-3">
      ...
      <div>
        ...
        <select
          value={aggregation}
          onChange={(event) => setAggregation(event.target.value as ChartAggregation)}
          className="w-full border border-gray-300 bg-white text-gray-900 rounded px-2 py-1.5 text-sm"
        >
