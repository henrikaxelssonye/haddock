...
import type { CanvasObject, ColumnSelection } from '../../types';
...
export function CanvasObjectWrapper({ obj }: CanvasObjectWrapperProps) {
  ...
  const isTableObject = obj.type === 'table';

  const isComposite = useMemo(() => {
    if (!isTableObject) {
      return false;
    }
    return getTablesFromSelections(obj.columnSelections).length > 1;
  }, [isTableObject, obj]);
      ...
      >
        ...
        <div className="flex items-center gap-1 relative shrink-0">
          <button
            ...
            title={obj.type === 'table' ? 'Configure columns' : 'Configure chart'}
          >
...
function CompositeColumnPickerInline({
  ...
  const isColumnSelected = (table: string, column: string) =>
    selected.some((cs) => cs.table === table && cs.column === column);

  const getSelectedCountForTable = (tableName: string) =>
    selected.filter((cs) => cs.table === tableName).length;
